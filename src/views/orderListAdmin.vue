<template>
  ...
  // Step 1: Filter orders by matching the hotel ID
  filteredOrdersByHotel() {
    const userHotelId = this.user.hotel; // Using optional chaining to prevent errors if hotel is null
    return this.getOrders.filter(order => order.hotel._id === userHotelId);
  },

  // Step 2: Apply additional search filters on top of the hotel filter
  filteredData() {
    const searchText = this.searchQuery.toLowerCase();
    return this.filteredOrdersByHotel.filter((order) => 
      order.orderCode.toString().includes(searchText) ||
      order.checkIn.toString().includes(searchText) ||
      order.checkOut.toString().includes(searchText) || 
      (order.userDetails.fullName && order.userDetails.fullName.toLowerCase().includes(searchText))
    );
  },
